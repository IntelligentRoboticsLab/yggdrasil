...
pub struct SeidrStates {
    ...
    pub focused_resource: Arc<Mutex<Option<String>>>,
}
...
impl Seidr {
    ...
    fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        let focused_resource = self.states.focused_resource.clone();

        let mut locked_focused_resource = focused_resource.lock().unwrap();
        {
            ...
            for name in resource_names.into_iter() {
                if let Some(data) = locked_resource_map.0.get_mut(&name) {
                    let followup_action =
                        add_editable_resource(ui, &name, data, &mut locked_focused_resource);
...
fn add_editable_resource(
    ...
    focused_resource: &mut Option<String>,
) -> Option<EditableResourceAction> {
    let mut followup_action = None;

    ui.vertical(|ui| {
        ui.label(egui::RichText::new(resource_name).heading());
        ui.add_space(10.0);
