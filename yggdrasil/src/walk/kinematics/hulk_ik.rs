...
use nidhogg::types::{JointArray, LeftLegJoints, RightLegJoints};

use crate::walk::{engine::Side, dnt_walk::FootOffset};

use super::{
    inverse::FootPosition,
    robot_dimensions::{HIP_TO_KNEE, KNEE_TO_ANKLE, ROBOT_TO_LEFT_PELVIS, ROBOT_TO_RIGHT_PELVIS},
};

pub fn leg_angles(
    left_foot: FootOffset,
    right_foot: FootOffset,
) -> (bool, LeftLegJoints<f32>, RightLegJoints<f32>) {
    let torso_offset = 0.02;
    // let left_foot_to_torso = super::inverse::foot_to_torso(Isometry3::from(ROBOT_TO_LEFT_PELVIS), 1f32, &left_foot);
    let left_foot_to_torso = calculate_foot_to_robot(
        ...
        left_foot.foot_lift,
        torso_offset,
        left_foot.hip_height,
    );
    let right_foot_to_torso = calculate_foot_to_robot(
        Side::Right,
        right_foot.clone(),
        0.0,
        right_foot.foot_lift,
        torso_offset,
        right_foot.hip_height,
    );
    // let right_foot_to_torso = super::inverse::foot_to_torso(Isometry3::from(ROBOT_TO_RIGHT_PELVIS), -1f32, &right_foot);
    let ratio = 0.5;
    let torso_to_left_pelvis = Isometry3::rotation(Vector3::x() * -1.0 * PI / 4.0)
        * Translation3::from(-ROBOT_TO_LEFT_PELVIS);
    ...
    let upper_leg = HIP_TO_KNEE.z.abs();
    let lower_leg = KNEE_TO_ANKLE.z.abs();
...
pub fn calculate_foot_to_robot(
    ...
    let hip_to_robot = match side {
        Side::Left => Isometry3::from(ROBOT_TO_LEFT_PELVIS),
        Side::Right => Isometry3::from(ROBOT_TO_RIGHT_PELVIS),
    };
