...
use openvino::{Blob, CNNNetwork, Core, ExecutableNetwork, InferRequest, Layout, Precision, TensorDesc};
use tyr::tasks::{compute::{ComputeDispatcher, ComputeTask}, task::Pollable};
use super::Model;
...
impl<M: Model> MLTask<M> {
    pub fn new(core: &mut Core, dispatcher: ComputeDispatcher) -> Self {
        Self {
...
impl<M: Model> MLModel<M> {
    ...
    fn new(core: &mut Core) -> Self {
        ...
        let exec = Mutex::new(core.load_network(&model, "CPU").expect("Failed to load executable network!"));

        let input_name = model.get_input_name(0).unwrap();
        let output_name = model.get_output_name(0).unwrap();

        Self {
            model, exec, input_name, output_name, _marker: PhantomData
        }
    }
...
impl<M: Model> MLInferRequest<M> {
    ...
    fn new(mut req: InferRequest, input: &[u8], input_name: &str, output_name: String) -> Self {
        ...
        Self {
            req, output_name, _marker: PhantomData
        }
    }
    ...
    fn infer(&mut self) -> Vec<u8> {      
        ...
        return self.req.get_blob(&self.output_name).unwrap()
            .buffer_mut().unwrap()
