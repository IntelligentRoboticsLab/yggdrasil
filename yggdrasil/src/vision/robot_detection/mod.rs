...
fn postprocess_detections(
    ...
    config: &RobotDetectionConfig,
    known_robots: &Vec<DetectedRobot>,
    ...
    filtered_boxes.extend(persisted);

    let nms_boxes = filtered_boxes
        .iter()
        .cloned()
        .map(
            |DetectedRobot {
    ...
    non_max_suppression(&nms_boxes, config.nms_threshold)
        .iter()
