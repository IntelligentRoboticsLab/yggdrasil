...
mod tests {
    ...
    use crate::{
        behavior2::behaviors::{Stand, Walk},
        motion::walk::engine::Step,
    };

    use super::*;

    pub fn change_behavior(mut commands: Commands, behavior: Res<State<BehaviorState>>) {
        println!("Changing behavior from {behavior:?}");
        match behavior.get() {
            BehaviorState::Walk => {
            ...
            BehaviorState::Stand => {
                commands.set_behavior(Walk {
                    step: Step::default(),
                    look_target: Some(Point3::origin()),
                });
            }
            _ => {}
        }
    ...
    pub fn print_behavior(behavior: Res<State<BehaviorState>>) {
    ...
    fn main() {
        let mut app = App::new();
        app.add_plugins(BehaviorEnginePlugin)
            .add_systems(Update, (change_behavior, print_behavior));
