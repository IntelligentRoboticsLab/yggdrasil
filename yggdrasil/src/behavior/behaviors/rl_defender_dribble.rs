...
pub struct RlDefenderDribbleBehaviorPlugin;

impl Plugin for RlDefenderDribbleBehaviorPlugin {
    fn build(&self, app: &mut App) {
        app.init_ml_model::<RlDefenderDribbleBehaviorModel>()
            .add_systems(
                Update,
                run_inference
                    .run_if(in_behavior::<RlDefenderDribbleBehavior>.and(task_finished::<Output>)),
            )
            .add_systems(
                Update,
                handle_inference_output
                    .after(run_inference)
                    .run_if(in_behavior::<RlDefenderDribbleBehavior>)
...
pub struct RlDefenderDribbleBehaviorConfig {
...
pub(super) struct RlDefenderDribbleBehaviorModel;

impl MlModel for RlDefenderDribbleBehaviorModel {
    type Inputs = ModelInput;
    type Outputs = ModelOutput;

    const ONNX_PATH: &'static str = "models/rl_defender_dribble_behavior.onnx";
}

#[derive(Resource)]
pub struct RlDefenderDribbleBehavior;

impl Behavior for RlDefenderDribbleBehavior {
    const STATE: BehaviorState = BehaviorState::RlDefenderDribbleBehavior;
}
...
fn run_inference(
    ...
    mut model_executor: ResMut<ModelExecutor<RlDefenderDribbleBehaviorModel>>,
...
fn handle_inference_output(
    ...
    step_context
        .request_walk(output.step * behavior_config.rl_defender_dribble.policy_output_scaling);
