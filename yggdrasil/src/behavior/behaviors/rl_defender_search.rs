...
pub struct RlDefenderSearchBehaviorPlugin;

impl Plugin for RlDefenderSearchBehaviorPlugin {
    fn build(&self, app: &mut App) {
        app.init_ml_model::<RlDefenderSearchBehaviorModel>()
            .add_systems(
                Update,
                run_inference
                    .run_if(in_behavior::<RlDefenderSearchBehavior>.and(task_finished::<Output>)),
            )
            .add_systems(
                OnEnter(BehaviorState::RlDefenderSearchBehavior),
            ...
            .add_systems(
                ...
                handle_inference_output
                    .after(run_inference)
                    .run_if(in_behavior::<RlDefenderSearchBehavior>)
...
pub struct RlDefenderSearchBehaviorConfig {
...
pub(super) struct RlDefenderSearchBehaviorModel;

impl MlModel for RlDefenderSearchBehaviorModel {
    type Inputs = ModelInput;
    type Outputs = ModelOutput;

    const ONNX_PATH: &'static str = "models/rl_defender_search_behavior.onnx";
}

#[derive(Resource)]
pub struct RlDefenderSearchBehavior;

impl Behavior for RlDefenderSearchBehavior {
    const STATE: BehaviorState = BehaviorState::RlDefenderSearchBehavior;
}
...
fn run_inference(
    ...
    mut model_executor: ResMut<ModelExecutor<RlDefenderSearchBehaviorModel>>,
...
fn handle_inference_output(
    ...
    step_context
        .request_walk(output.step * behavior_config.rl_defender_search.policy_output_scaling);

    let observe_config = &behavior_config.rl_defender_search;
    look_around(
        &mut nao_manager,
