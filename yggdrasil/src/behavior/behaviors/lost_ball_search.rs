...
impl Plugin for LostBallSearchBehaviorPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Update, observe.run_if(in_behavior::<LostBallSearch>))
            .add_systems(OnEnter(BehaviorState::Observe), reset_lost_ball_search_starting_time)
            .insert_resource(LostBallSearchStartingTime(Instant::now()));
    }
}

fn reset_lost_ball_search_starting_time(mut lost_ball_search_starting_time: ResMut<LostBallSearchStartingTime>) {
